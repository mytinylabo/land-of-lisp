...
;; (to-file "die.svg" (svg 100 100 (draw-die-svg 50 50 '(255 0 0))))
...
;; (to-file "tile.svg" (svg 300 300 (draw-tile-svg 0 0 0 '(0 3) 100 150 '(255 0 0) nil)))

(defparameter *dice-colors* '((255 63 63) (63 63 255)))
...
(defun draw-board-svg (board chosen-tile legal-tiles)
    (loop for y below *board-size*
        do (loop for x below *board-size*
                 ...
                 for col = (brightness (nth (first hex) *dice-colors*)
                                       (* -15 (- *board-size* y)))
...
;; (to-file "board.svg" (svg *board-width* *board-height* (draw-board-svg (gen-board) nil nil)))
...
(defun web-announce-winner (board)
    ...
    (let ((w (winners board)))
        (if (> (length w) 1)
            (format t "The game is a tiel bewtoeen ~a" (mapcar #'player-letter w))
