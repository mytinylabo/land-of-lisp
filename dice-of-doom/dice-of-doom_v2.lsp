...
;; Don't call it with an entire game tree!
(defun force-tree (tree)
    (list (car tree)
          (cadr tree)
          (let ((moves (take-all (caddr tree))))
            (if moves
                (mapcar (lambda (move)
                            (list (car move)
                                (force-tree (cadr move))))
                    moves)
                nil))))
...
;; (to-file "tree.txt"
;;     (princ (force-tree
;;                 (limit-tree-depth (game-tree (gen-board) 0 0 t)
;;                                   4))))

(defparameter *ai-level* 4)

(defun handle-computer (tree)
    ;; (print-moves (caddr tree))
